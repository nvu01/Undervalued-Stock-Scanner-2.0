...
def get_files():
    ...
    '''
    all_files = []
    for root, dirs, files in os.walk('Downloaded CSV Files'):
...
def mean_std(group):
    ...
    filtered = group[(group >= lower) & (group <= upper)]
    means = filtered.mean()
    stds = filtered.std(ddof=0)
...
def scan(df):
    ...
    '''
    # Apply the first preliminary criteria
...
def main():
    ...
    for i, file in enumerate(all_files):
        ...
        scored_df.to_excel('Results\\' + filename + '.xlsx', index=False)
        stats.to_excel('Industry Means\\' + filename + '.xlsx', index=True)
