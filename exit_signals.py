...
from etl import get_files, data_cleaning, get_grouped_stats
import pandas as pd
...
def get_current_pos():
    ...
    files = glob.glob(os.path.join('Position Statement', "*"))
...
def get_all_stock_data(all_files, metric_cols, pos):
    ...
    Filter only the data for active stocks in position statement
    '''
    ...
    full_df.drop(columns=full_df.filter(like='_std').columns, axis=1, inplace=True)
...
def scan(df):
    ...
    scanned_df = df.loc[~df['Market Cap Group'].isna(), :]
    ...
    scanned_df = scanned_df[
        ['Symbol', 'Industry', 'Overvaluation', 'Bad Quality', 'P/FCF', 'P/FCF_mean', 'P/B', 'P/B_mean', 'P/E',
...
def main():
    ...
    out_of_scanner.to_excel(f'Exit Signals\\Unscannable\\{timestamp}_OutOfScanner.xlsx', index=False)

    output_path = os.path.join(f'Exit Signals\\Scannable\\{timestamp}_ExitSignals.xlsx')
    out_put(scanned_df, output_path)
